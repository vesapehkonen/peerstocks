...
from opensearch_client import build_opensearch_client, run_opensearch_query

os_client = build_opensearch_client()


def fetch_earnings(ticker: str):
    query = {"size": 1000, "query": {"bool": {"must": [{"term": {"ticker": ticker}}]}}, "sort": [{"end_date": "asc"}]}
    data = run_opensearch_query(os_client, "earnings_data", query)
    return data
...
def find_existing_trading_day(ticker, date_str):
    ...
    for _ in range(7):
        ...
        query = {"query": {"bool": {"must": [{"term": {"ticker": ticker}}, {"term": {"date": check_date}}]}}, "size": 1}
