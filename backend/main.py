...
import json
from opensearch_client import run_opensearch_query, get_opensearch_client, run_opensearch_raw
...
def generate_stock_summary(ticker: str):
  ...
  earnings = fetch_earnings(ticker.upper())

  if not earnings:
    return {"summary": "No earnings data available to summarize."}

  # Take last 4 earnings entries
  lines = []
  for doc in earnings[-4:]:
    src = doc["_source"]
    date = src.get("end_date", "N/A")
  ...
  prompt = f"""
You are a financial assistant analyzing stock performance for a retail investor. Based on the following recent financial data for {ticker}, write a clear and concise summary of key trends.

Financial data:
{metrics_text}

If relevant, highlight any seasonal patterns (e.g., strong Q4 results), recent volatility, or sustained growth. If provided, refer to the P/E ratio to comment on valuation (but do not give investment advice). Use plain English. Limit your response to 3–4 sentences and under 100 words. Avoid bullet points.
"""
  try:
    response = openai_client.chat.completions.create(model=settings.OPENAI_MODEL,
                                                     messages=[{
                                                         "role": "user",
                                                         "content": prompt
                                                     }],
    ...
    if full_summary.count(".") > 4 or len(full_summary.split()) > 120:
      ...
      retry = openai_client.chat.completions.create(model=settings.OPENAI_MODEL,
                                                    messages=[{
                                                        "role": "user",
...
def get_price_history(ticker: str):
  query = {"size": 5000, "query": {"bool": {"must": [{"term": {"ticker": ticker.upper()}}]}}, "sort": [{"date": "asc"}]}
  data = run_opensearch_query(http_client, 'stock_prices', query)
...
def get_stock_data(ticker: str):
  ...
  price_data = run_opensearch_query(http_client, 'stock_prices', price_query)
  ...
  return {"quarterly": result, "daily_prices": daily_prices}
...
def eps_ttm_points(http_client, ticker: str, max_years: int = 6) -> List[Dict[str, Any]]:
  """
...
def eps_ttm_points(http_client, ticker: str, max_years: int = 6) -> List[Dict[str, Any]]:
    #
    #Returns a list of points: [{date: 'YYYY-MM-DD', eps_ttm: float}], one per reported quarter,
    #where eps_ttm is the rolling sum of the last 4 quarters' diluted_eps (fallback to basic_eps).
    #
    since = (datetime.now(timezone.utc) - timedelta(days=365 * max_years)).date().isoformat()
    hits = run_opensearch_query(http_client, 'earnings_data', {
        "size": 2000,  # enough quarters
        "sort": [{"end_date": {"order": "asc"}}],
        "_source": ["end_date", "diluted_eps", "basic_eps", "ticker"],
        "query": {"bool": {"filter": [
            {"term": {"ticker": ticker}},
            {"range": {"end_date": {"gte": since}}}
        ]}},
    })
    # build quarter EPS list
    q = []
    for h in hits:
        s = h["_source"]
        date = s["end_date"][:10]
        eps = s.get("diluted_eps")
        if eps is None:
            eps = s.get("basic_eps")
        if eps is None:
            continue
        q.append({"date": date, "eps": float(eps)})

    # rolling sum of last 4 quarters for EPS TTM
    out = []
    window = []
    total = 0.0
    for pt in q:
        window.append(pt["eps"])
        total += pt["eps"]
        if len(window) > 4:
            total -= window.pop(0)
        if len(window) == 4:
            out.append({"date": pt["date"], "eps_ttm": total})
    return out
"""


def prices_last_days_with_eps_pe(http_client, ticker: str, days: int) -> List[Dict[str, Any]]:
  since = (datetime.now(timezone.utc) - timedelta(days=days)).date().isoformat()
  # daily close (you already had this query)
  price_hits = run_opensearch_query(
      http_client, 'stock_prices', {
          "size": 2000,
  ...
  eps_points = eps_ttm_points(http_client, ticker)
...
def latest_two_prices(http_client, ticker):
  query = {
      "size": 2,
      "sort": [{
          "date": {
...
def prices_last_days(http_client, ticker, days):
  since = (datetime.now(timezone.utc) - timedelta(days=days)).date().isoformat()
  query = {
      "size": 2000,
      "sort": [{
...
def hi_lo_52w(http_client, ticker):
  ...
  resp = run_opensearch_raw(http_client, 'stock_prices', query)
...
def latest_summary_pe(http_client, ticker):
  query = {
      "size": 1,
      "sort": [{
          "ttm_pe_ratio": {
  ...
  hits = run_opensearch_query(http_client, 'stock_summary', query)
  if not hits:
    return None
  return hits[0]["_source"].get("ttm_pe_ratio")


def latest_earnings(http_client, ticker):
  # newest by end_date
  query = {
      "size": 1,
      "sort": [{
  ...
  hits = run_opensearch_query(http_client, 'earnings_data', query)
...
def build_payload_for_ticker(http_client, ticker):
  t = ticker.upper()
  # prices
  two = latest_two_prices(http_client, t) or []
  ...
  #series = prices_last_days(http_client, t, days=200)
  series = prices_last_days_with_eps_pe(http_client, t, days=2000)  # each point has {date, close, eps, pe}

  # 52w hi/lo
  hilo = hi_lo_52w(http_client, t) or {}
  ...
  ttm_pe = latest_summary_pe(http_client, t) or pe_latest

  # earnings snapshot
  earn = latest_earnings(http_client, t) or {}
...
def get_stocks(tickers: List[str] = Query(..., description="Repeat param, e.g. ?tickers=AAPL&tickers=MSFT")):
  ...
  out: List[Dict[str, Any]] = []
  for t in tickers:
    out.append(build_payload_for_ticker(http_client, t.strip()))
  return out
